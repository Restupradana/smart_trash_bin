<x-app-layout title="Dashboard User">
    ...
    <div class="flex-grow-1 bg-light p-4">
        ...
        {{-- <a href="{{ route('user.notifikasi.form') }}"
           class="bg-green-600 text-white px-4 py-2 rounded inline-block mb-4">
            Kirim Notifikasi Sampah Penuh
        </a>

        <ul class="bg-green-600 shadow rounded p-4 mb-5">
            @foreach($notifikasis as $notif)
                <li class="border-b py-2">
                    Lokasi: {{ $notif->lokasi }} |
                    Status: {{ $notif->dikonfirmasi ? 'Dikonfirmasi' : 'Menunggu' }}
                </li>
            @endforeach
        </ul> --}}
